...
const CONFIG = {
    // API endpoints
    apiEndpoint: '/predict',
    imageApiEndpoint: '/predict-image',
...
function displayResults(data, type) {
    ...
    if (isFake && (verdict === 'corroborated' || (verdict === 'partially-verified' && trustedCount >= 1))) {
        // ML says Fake but trusted sources cover the story
        conflictHTML = `
            <div style="
            ...
            ">
                ...
                <p style="margin: 0; font-size: 0.85rem; color: var(--text-secondary); line-height: 1.5;">
                    The AI model flagged this as <strong>Fake</strong>, but <strong>${trustedCount} trusted news source(s)</strong> are reporting similar content.
                    The web sources suggest this news is likely <strong>legitimate</strong>. We recommend checking the source links below for your own verification.
                </p>
            </div>
        `;
    } else if (!isFake && (verdict === 'low-coverage' || verdict === 'inconclusive')) {
        // ML says Real but no sources found
        conflictHTML = `
            <div style="
                margin-top: 1.25rem; padding: 1rem 1.25rem; border-radius: 0.75rem;
                background: linear-gradient(135deg, #fef3c720, #fbbf2420);
            ...
            ">
                ...
                <p style="margin: 0; font-size: 0.85rem; color: var(--text-secondary); line-height: 1.5;">
                    The AI model classified this as <strong>Real</strong>, but very few web sources were found covering this story.
    ...
    resultsContent.innerHTML = `
        ...
        <div class="result-explanation">
            <h4><i class="fas fa-robot"></i> AI Model Analysis</h4>
        ...
        <div style="margin-top: 1.5rem; padding: 1rem; background: var(--bg-tertiary); border-radius: 0.75rem; font-size: 0.875rem; color: var(--text-secondary);">
            <p><strong>Note:</strong> This analysis combines AI model predictions with real-time web source verification. When there is a conflict, the source verification results from trusted outlets are generally more reliable.</p>
        </div>
    `;
